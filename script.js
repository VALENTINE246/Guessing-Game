...
const showTipBox = document.getElementById('tip-confirm-box')
const confirmTextTip = document.getElementById('confirm-text-box')
const closeTipBox = document.getElementById('close-tip-box')
...
let currentGuessLimit = localStorage.getItem('currentGuessLimit') ? parseInt(localStorage.getItem('currentGuessLimit')) : 2;
let currentGuessrange = localStorage.getItem('currentGuessrange') ? parseInt(localStorage.getItem('currentGuessrange')) : 2;
let currentLevelGuessLimit = localStorage.getItem('currentLevelGuessLimit') ? parseInt(localStorage.getItem('currentLevelGuessLimit')) : 5;
  ...
  if (inputElement.value) {
    ...
    if (guessValue >= 1 && guessValue <= levelNoOfGuess) {
      ...
      if (guessValue === randomNumber) {
        ...
        levelNoOfGuess += 2;
        currentLevelGuessLimit += 2
        localStorage.setItem('currentLevelGuessLimit', currentLevelGuessLimit)
        ...
        localStorage.setItem('currentGuessLimit', currentGuessLimit)
        availableGuessElement.innerHTML = guessLimit


        showLimit = guessLimit
        currentLevelGuessLimit = currentGuessLimit
        localStorage.setItem(' currentLevelGuessLimit',  currentLevelGuessLimit)
        localStorage.setItem('showLimit', showLimit)
...
tipBtn.addEventListener('click', () => {
  if (randomNumber !== 1) {
    if(tipNumber && showLimit < 2) {
      showTipBox.style.display = 'flex'
      randomTip = Math.floor(Math.random() * 2) + 1;
      if (randomTip === 1) {
        confirmTextTip.innerHTML = `Answer is either ${randomNumber - 1} or ${randomNumber}`
      } else if (randomTip === 2) {
        confirmTextTip.innerHTML = `Answer is either ${randomNumber} or ${randomNumber + 1}`      
      }
      tipNumber -= 1
      localStorage.setItem('tipNo', tipNumber)
      showTipNo.innerHTML = tipNumber
...
closeTipBox.addEventListener('click', () => {
  showTipBox.style.display = 'none'
})
...
// handle to clear all data including local storage.
...
okClear.addEventListener('click', (e) => {
  ...
  localStorage.removeItem('guessLimit', guessLimit)
  localStorage.removeItem('showLimit', showLimit)

  localStorage.removeItem('currentGuessLimit', currentGuessLimit)
  localStorage.removeItem('currentGuessrange', currentGuessrange)
  localStorage.removeItem('currentLevelGuessLimit', currentLevelGuessLimit)

  currentGuessLimit = 2;
  currentGuessrange = 2;
  currentLevelGuessLimit = 5;

  localStorage.setItem('currentGuessLimit', currentGuessLimit)
  localStorage.setItem('currentGuessrange', currentGuessrange)
  localStorage.setItem('currentLevelGuessLimit', currentLevelGuessLimit)
